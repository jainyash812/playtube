...
import {videoData, demoThumbnailUrl, demoVideoUrl, demoVideoTitle, demoChannelUrl, demoChannelTitle } from "../helper/constants";
import VideoCard from './VideoInformation';
const VideoInformation = () => {
  const [videoDetail, setVideoDetail] = useState(videoData[0]);
 ...
 useEffect(() => {
  ...
  if(!!videoDetail && !videoDetail.snippet){return <>Loading .....</>}
  return (
           ...
           <Box sx={{display:'flex',flexDirection:'row'}}>
            ...
            <Box  sx={{marginTop:'1.3rem'}}>
                {!!videos && videos.map((item,idx)=>{
                    return <>
                        <Box key={`item${idx}`} sx={{display:"flex",flexDirection:'column',alignItems:'center',flexWrap:'wrap'}}>
                            <VideoCard videoDetailInfo={item} />
                        </Box>
