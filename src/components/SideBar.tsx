...
const SideBar = ({categorySelected,changeSelectedCategory,openStatus,toggleSideBar}:SideBarProps) =>{
   ...
   return(<Paper sx={{position:'absolute',top:0,left:0,width:"15rem",height:"100vh",padding:'0 20px'}}>
            ...
            <Stack direction="row" sx={{overflowY:"auto",display:'flex',flexDirection:'column',marginTop:"10px"}}>
                {categories.map((item)=>{
                    return(<button key={`${item.name}`} onClick={()=>changeSelectedCategory(item.name)} className={categorySelected===item.name?"sidebar-button active":"sidebar-button"}>
